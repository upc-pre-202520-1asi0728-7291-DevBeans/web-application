"use client"

import type React from "react"
import { useState } from "react"
import Link from "next/link"
import { usePathname } from "next/navigation"
import { Button } from "@/components/ui/button"
import { Coffee, LayoutDashboard, Package, FileText, Settings, Menu, LogOut } from "lucide-react"
import { useAuth } from "@/hooks/contexts/auth-context"
import type { ProducerProfile } from "@/lib/services/user.service"

const navigation = [
  { name: "Dashboard", href: "/dashboard/producer", icon: LayoutDashboard },
  { name: "Lotes", href: "/dashboard/producer/batches", icon: Package },
  { name: "Reportes", href: "/dashboard/producer/reports", icon: FileText },
  { name: "Configuración", href: "/dashboard/producer/settings", icon: Settings },
]

export function ProducerLayout({ children }: { children: React.ReactNode }) {
  const pathname = usePathname()
  const { user, profile, logout, loading } = useAuth()
  const [sidebarOpen, setSidebarOpen] = useState(false)

  // Castear el perfil a ProducerProfile
  const producerProfile = profile as ProducerProfile | null

  const handleLogout = async () => {
    await logout()
  }

  // Mostrar loading mientras se cargan los datos
  if (loading) {
    return (
        <div className="min-h-screen bg-gray-50 flex items-center justify-center">
          <div className="text-center">
            <Coffee className="h-12 w-12 text-amber-700 mx-auto mb-4 animate-pulse" />
            <p className="text-gray-600">Cargando...</p>
          </div>
        </div>
    )
  }

  return (
      <div className="min-h-screen bg-gray-50">
        {/* Mobile sidebar backdrop */}
        {sidebarOpen && (
            <div className="fixed inset-0 z-40 bg-gray-600 bg-opacity-75 lg:hidden" onClick={() => setSidebarOpen(false)} />
        )}

        {/* Sidebar */}
        <div
            className={`fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-gray-200 transform transition-transform duration-300 ease-in-out lg:translate-x-0 ${
                sidebarOpen ? "translate-x-0" : "-translate-x-full"
            }`}
        >
          <div className="flex h-full flex-col">
            {/* Logo */}
            <div className="flex h-16 items-center gap-2 border-b border-gray-200 px-6">
              <Coffee className="h-8 w-8 text-amber-700" />
              <span className="text-xl font-bold text-gray-900">BeanDetect AI</span>
            </div>

            {/* Navigation */}
            <nav className="flex-1 space-y-1 px-3 py-4">
              {navigation.map((item) => {
                const isActive = pathname === item.href
                return (
                    <Link
                        key={item.name}
                        href={item.href}
                        className={`flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium transition-colors ${
                            isActive ? "bg-amber-50 text-amber-700" : "text-gray-700 hover:bg-gray-50 hover:text-gray-900"
                        }`}
                        onClick={() => setSidebarOpen(false)}
                    >
                      <item.icon className="h-5 w-5" />
                      {item.name}
                    </Link>
                )
              })}
            </nav>

            {/* User section - DATOS REALES DEL BACKEND */}
            <div className="border-t border-gray-200 p-4">
              <div className="mb-3">
                <p className="text-sm font-medium text-gray-900">
                  {producerProfile
                      ? `${producerProfile.first_name} ${producerProfile.last_name}`
                      : user?.email || 'Usuario'}
                </p>
                <p className="text-xs text-gray-500">
                  {producerProfile?.farm_name || 'Cargando...'}
                </p>
              </div>
              <Button
                  variant="outline"
                  className="w-full justify-start gap-2 text-gray-700 hover:text-gray-900 bg-transparent"
                  onClick={handleLogout}
              >
                <LogOut className="h-4 w-4" />
                Cerrar Sesión
              </Button>
            </div>
          </div>
        </div>

        {/* Main content */}
        <div className="lg:pl-64">
          {/* Top bar */}
          <div className="sticky top-0 z-10 flex h-16 items-center gap-4 border-b border-gray-200 bg-white px-4 lg:px-8">
            <button className="lg:hidden" onClick={() => setSidebarOpen(true)} aria-label="Open sidebar">
              <Menu className="h-6 w-6" />
            </button>
            <h1 className="text-lg font-semibold text-gray-900">
              {navigation.find((item) => item.href === pathname)?.name || "Dashboard"}
            </h1>
          </div>

          {/* Page content */}
          <main className="p-4 lg:p-8">{children}</main>
        </div>
      </div>
  )
}