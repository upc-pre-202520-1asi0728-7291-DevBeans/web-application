...
import { userService, CooperativeProfile } from "@/lib/services/user.service"
...
export function CooperativeLayout({ children }: { children: React.ReactNode }) {
  ...
  const [cooperativeInfo, setCooperativeInfo] = useState<CooperativeProfile | null>(null)

  useEffect(() => {
    loadCooperativeInfo()
  }, [])

  const loadCooperativeInfo = async () => {
    try {
      const user = authService.getUser()
      if (user && user.user_type === 'COOPERATIVE') {
        const profile = await userService.getProfile(user.id) as CooperativeProfile
        setCooperativeInfo(profile)
      }
    } catch (error) {
      console.error('Error loading cooperative info:', error)
    }
  }
            ...
            <div className="border-t border-gray-200 p-4">
              <div className="mb-3">
                <p className="text-sm font-medium text-gray-900">
                  {cooperativeInfo?.cooperative_name || 'Cooperativa'}
                </p>
